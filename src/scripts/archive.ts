interface CatalogPhoto {
  id: string; index: number; thumbnail: string; archiveImage: string; width: number; height: number;
  ...
  featured: boolean; editorialOrder: number | null; altText: string;
}
...
if (root) {
  ...
  const frameMarkup = (photo: CatalogPhoto) => `
    <button class="archive-frame is-${photo.orientation ?? 'unknown'}${photo.featured ? ' is-selected' : ''}" type="button" data-photo-id="${photo.id}" aria-label="Open frame ${photo.index}">
  ...
  const showViewer = (index: number, origin?: HTMLElement) => {
    ...
    if (!photo) return;
    viewerImage.src = photo.archiveImage;
  ...
  fetch('/data/photo-catalog.json').then((response) => response.json()).then((data) => {
    catalog = data.photos;
    const selected = catalog.filter((photo) => photo.featured).sort((a, b) => Number(a.editorialOrder) - Number(b.editorialOrder));
    filtered = [...selected, ...catalog.filter((photo) => !photo.featured)];
  ...
  viewer.addEventListener('close', () => { delete document.documentElement.dataset.viewerOpen; viewerImage.removeAttribute('src'); viewerOrigin?.focus({ preventScroll: true }); viewerOrigin = null; });
