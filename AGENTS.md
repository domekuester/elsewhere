...
- Publication permission and licensing permission are separate. `publicationStatus` decides whether a photograph may appear; `rightsStatus` in [src/data/rights.ts](src/data/rights.ts) decides what may be offered when someone asks to use it. Uncertainty must never resolve into a commercial claim: only an owner-confirmed state may describe real availability, and `modelReleaseStatus`, `propertyReleaseStatus` and `rightsNotesInternal` never leave the private layer. Never invent clearance, releases, prices, clients, testimonials, or travel recommendations.
- The commercial layer stays at the edges. Studio, Licensing and enquiry actions never enter Home's photographic sequence, People, Black & White, or the immersive viewer's image space — the viewer's single licensing link belongs in the secondary caption row. Every enquiry affordance is gated on a real destination — a configured Formspree licensing form (`PUBLIC_FORMSPREE_LICENSING_FORM_ID`) or a verified `PUBLIC_CONTACT_EMAIL`; with neither, the path is hidden rather than shipping a dead form. A licensing enquiry carries the frame's canonical public id and is resolved back through the published catalog by [src/data/licensing.ts](src/data/licensing.ts): the URL requests a lookup and never supplies content, so an unknown, private or not-for-license reference degrades to a general enquiry. Verify with `npm run validate:licensing`. Publish no placeholder or "coming soon" surface: a route that has no real content must not build at all.
